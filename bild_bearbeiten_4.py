...
class MainWindow(QMainWindow):
    ...
    def init_files_tab(self):
        ...
        setup_button.clicked.connect(self.open_config_editor)
    ...
    def scan_image_thread(self, img_name_local, batch_scanning=False):
        ...
        #old hard coded save path fo csv
    ...
    def create_folder(self):
        ...
        # check config file, create if not
        if not os.path.exists(self.config_file):
            with open(self.config_file, 'w') as f:
                json.dump({
                    "csv_path": self.csv_path,
                    "csv_edit": self.csv_edit,
                    "bilder_path": self.bilder_path
                }, f, indent=4)
    ...
    def load_config_file(self):
        ...
        try:
            with open(self.config_file, 'r') as f:
                ...
                self.csv_edit = config.get("csv_edit", self.csv_edit)
                self.bilder_path = config.get("bilder_path", self.bilder_path)
...
class ConfigEditor(QWidget):
    def __init__(self, main_window):
        ...
        self.main_window = main_window # store reference class  mainWindow
        self.text_edit = QTextEdit()
        self.save_button = QPushButton("Speichern")

        # ordner auswaehlen
        self.csv_path_button = QPushButton("Ordnerspeicherort der CSV")
        self.csv_edit_button = QPushButton("Ordnerspeicherort der Bearbeitung")
        self.bilder_path_button = QPushButton("Ordnerspeicherort der Bilder")

        self.config_file = 'config.json' # dir of config

        layout = QVBoxLayout()
        layout.addWidget(self.csv_path_button)
        layout.addWidget(self.csv_edit_button)
        layout.addWidget(self.bilder_path_button)
        ...
        self.csv_path_button.clicked.connect(lambda: self.set_path("csv_path"))
        self.csv_edit_button.clicked.connect(lambda: self.set_path("csv_edit"))
        self.bilder_path_button.clicked.connect(lambda: self.set_path("bilder_path"))
    ...
    def load_config(self):
        try:
            ...
            QMessageBox.warning(self, "Error", "Falsche JSON format oder Konfiguration noch nicht eingerichtet")
            self.text_edit.setPlainText('{}')
    ...
    def set_path(self, key):
        directory = QFileDialog.getExistingDirectory(self, f"Select {key} folder")
        if directory:
            try:
                # Load existing config
                config = json.loads(self.text_edit.toPlainText())
                config[key] = os.path.abspath(directory)

                # Update GUI text
                self.text_edit.setPlainText(json.dumps(config, indent=4))

                # Set value on main window
                if hasattr(self.main_window, key):
                    setattr(self.main_window, key, os.path.abspath(directory))

            except json.JSONDecodeError:
                QMessageBox.warning(self, "Error", "Konfiguration noch nicht eingerichtet")
